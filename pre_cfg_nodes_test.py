...
class ExlAttentionNode:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "model": ("MODEL",),
                "scale": ("FLOAT", {"default": 2,  "min": -1.0, "max": 10.0, "step": 1/2, "round": 1/100}),
    ...
    def patch(self, model, scale, version="v1", start_at_sigma=999999999.9, end_at_sigma=0.0, enabled=True, attention="cross", unet_block="middle", unet_block_id=0):
        attn = {"both":["attn1","attn2"],"self":["attn1"],"cross":["attn2"]}[attention]
        ...
        def exl_attention(q, k, v, extra_options, mask=None):
            sigma = extra_options['sigmas'][0]
            if sigma > start_at_sigma or sigma <= end_at_sigma:
                return default_attention(q, k, v, extra_options['n_heads'], mask)
            else:
                if version == "v1":
                    ...
                    first_attention  = default_attention(q, k, v, extra_options['n_heads'], mask)
                    noise_like_q = q + (q - normlike(torch.rand_like(q),q))
                    second_attention = normlike(default_attention(noise_like_q, k, v, extra_options['n_heads'], mask), first_attention) * scale
                    return second_attention
                elif version == "v3":
                    noise_like_q = q + (q - normlike(torch.rand_like(q),q))
                    first_attention = default_attention(noise_like_q, k, v, extra_options['n_heads'], mask) * scale
                    return first_attention
                elif version == "v4":
                    noise_like_q = q + (q - normlike(torch.rand_like(q),q)) * scale
                    first_attention = default_attention(noise_like_q, k, v, extra_options['n_heads'], mask)
                    return first_attention
                elif version == "v5":
                    q, k = q + (q - normlike(k,q)) * scale, k + (k - normlike(q,k)) * scale
                    first_attention = default_attention(q, k, v, extra_options['n_heads'], mask)
                    return first_attention
                elif version == "v6":
                    q = q + (q - normlike(v,q)) * scale
                    first_attention = default_attention(q, k, v, extra_options['n_heads'], mask)
                    return first_attention
                elif version == "v7":
                    first_attention  = default_attention(q, k, v, extra_options['n_heads'], mask)
                    noise_like_q = q + (q - normlike(torch.rand_like(q), q)) * scale
                    second_attention = normlike(default_attention(noise_like_q, k, v, extra_options['n_heads'], mask), first_attention)
                    return second_attention
                elif version == "v8":
                    first_attention  = default_attention(q, k, v, extra_options['n_heads'], mask)
                    noise_like_q = normlike(q + (q - normlike(torch.rand_like(q),q)) * scale, q)
                    second_attention = normlike(default_attention(noise_like_q, k, v, extra_options['n_heads'], mask), first_attention)
                    return second_attention
                elif version == "v9":
                    noise_like_q = q + (q - normlike(torch.rand_like(q),q)) * scale
                    ...
                    first_attention = default_attention(q * scale, k, v, extra_options['n_heads'], mask)
                    return first_attention
        for att in attn:
            m.model_options = comfy.model_patcher.set_model_options_patch_replace(m.model_options, exl_attention, att, unet_block, unet_block_id)
...
class normalize_std_pre_cfg_node:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {"model": ("MODEL",),
                             "divide" :  ("BOOLEAN", {"default": True}),
                             "squared_std" : ("BOOLEAN", {"default": True}),
                             "squared_ratio" : ("BOOLEAN", {"default": True}),
                             "enabled" : ("BOOLEAN", {"default": True})
                             }}
    RETURN_TYPES = ("MODEL",)
    FUNCTION = "patch"
    ...
    def patch(self, model, divide, squared_std, squared_ratio, enabled):
        @torch.no_grad()
        def pre_cfg_patch(args):
            ...
            for b in range(len(conds_out[0])):
                ...
                if divide:
                    ...
                    pred_std_cond   = pred_std / conds_out[0][b].std()
                    pred_std_uncond = pred_std / conds_out[1][b].std()
                if squared_ratio:
