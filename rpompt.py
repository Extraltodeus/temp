...
"""
This script is intended for use with sexual/explicit models. You may get decent results at random with non-explicit
models, but the image will absolutely not match the generated prompt. Even with explicit models, the likelihood of the
image perfectly matching with the prompt is quite low.

Add, remove, or edit tokens to your liking, but please take note of the comments above each set. Only one token is
selected from a set per prompt (except `embeddingToken`, if multiple tokens are provided).

This script only generates the positive prompt. It is recommended that you provide a good negative prompt in your
txt2img tab.
"""

# Set the number of prompts to generate. Setting this too high may result in an async timeout in webui.
numPrompts = 10

# Set any of these to `True` to enable artist, art style, details, and beautifier randomization
# Artists is only supported on SD 1.4/1.5.
# ArtStyle, Details, and Beautifier can lead to SERIOUSLY random and even mangled results. Use at your own risk.
useArtists = False
useArtStyle = False
useDetails = False
useBeautifier = False

# Set this to true to use TI embedding or custom DB activation tokens
useEmbedding = True

# Set TI/DB activation tokens. Ex: Using the gape60 or AnythingGape model, "anal gape" is an activator token.
# You MUST edit this to suit any activators that your models/embeddings use, or set `useEmbedding` to `False`
embeddingToken = ["anal gape", "laura croft"]

# Optional generic keywords that are added towards the beginning of the prompt as individual tokens not part of the
# "scene" sentence.
...
# Type of woman
type = ("catgirl", "steamy", "mature", "hot", "foxgirl", "slime girl", "nymph", "angel", "batgirl",
...
def createPrompts(promptcount):
    ...
    for i in range(promptcount):
        ...
        if useEmbedding is True:
            for y in range(len(embeddingToken)):
                prompts = prompts + "(" + embeddingToken[y] + ":" \
                    + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + "), "

        if useBeautifier is True:
            prompts = prompts + "(" + getRandomStringFromList(beautifier) + ":" \
                + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + "), "

        prompts = prompts \
            + "(" + getRandomStringFromList(distance) + ":" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + "), " \
            + "(" + getRandomStringFromList(type) + ":" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + ") " \
            + "woman with " + "(" + getRandomStringFromList(bSize) + " breasts:" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) \
            + ") and (" + getRandomStringFromList(hairLength) + ":" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) \
            + " " + getRandomStringFromList(hairColor) + " hair:" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + ") " \
            + "and (" + getRandomStringFromList(eyeColor) + " eyes:" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + ") " \
            + "(" + getRandomStringFromList(accessory) + ":" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + ") " \
            + "(" + getRandomStringFromList(action) + ":" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + ") " \
            + "with (" + getRandomStringFromList(perp) + ":" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + ") " \
            + "(" + getRandomStringFromList(place) + ":" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + "), " \
            + "(" + getRandomStringFromList(expression) + " expression:" \
            + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + "), " \
            + "(nsfw:" + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + "), " \
            + "(perfect face:" + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + "), " \
            + "(perfect fingers:" + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + ")"

        if useDetails is True:
            prompts = prompts + ", (" \
                + getRandomStringFromList(details) + ":" + "{:.2f}".format(random.uniform(lower_bound, multiplier)) \
                + ")"

        if useArtStyle is True:
            prompts = prompts + ", (" \
                + getRandomStringFromList(artStyle) \
                + ":" + "{:.2f}".format(random.uniform(lower_bound, multiplier)) + ")"

        if useArtists is True:
            prompts = prompts + ", by (" \
